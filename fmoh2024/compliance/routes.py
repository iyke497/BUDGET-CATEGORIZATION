...
def api_service_level_breakdown():
    ...
    primary_services = db.session.query(
        SurveyResponse.primary_health_care,
        func.count().label('count')
    ).filter(
        SurveyResponse.fiscal_year == fiscal_year,
        SurveyResponse.is_matched == True,
    ...
    secondary_services = db.session.query(
        SurveyResponse.secondary_health_care,
        func.count().label('count')
    ).filter(
        SurveyResponse.fiscal_year == fiscal_year,
        SurveyResponse.is_matched == True,
    ...
    tertiary_services = db.session.query(
        SurveyResponse.tertiary_health_care,
        func.count().label('count')
    ).filter(
        SurveyResponse.fiscal_year == fiscal_year,
        SurveyResponse.is_matched == True,
...
def api_dashboard_stats():
    ...
    outcomes = db.session.query(
        ...
        func.count().desc()
    ).limit(6).all()  # Get top 6 to allow for "Other" category
    ...
    return jsonify({
        ...
        'compliance_distribution': {
            'labels': ['High Compliance (≥80%)', 'Medium Compliance (50-79%)', 
                      'Low Compliance (1-49%)', 'No Compliance (0%)'],
            'values': [tier_counts['high'], tier_counts['medium'], 
                      tier_counts['low'], tier_counts['zero']],
