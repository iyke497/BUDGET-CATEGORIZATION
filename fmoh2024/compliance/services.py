...
class ComplianceService:
    ...
    def get_focus_agencies(cls, fiscal_year='2024'):
        ...
        return MinistryAgency.query.filter(
    ...
    def get_summary_stats(cls, fiscal_year='2024'):
        ...
        overall_compliance = (total_categorized / total_projects * 100) if total_projects > 0 else 0
    ...
    def get_agency_project_details(cls, agency_id, fiscal_year='2024'):
        ...
        for project, response in projects:
            project_details.append({
                ...
                'appropriation': float(project.appropriation) if project.appropriation else 0,
                'status': project.project_status,
                'is_categorized': response is not None,
                'categorization': {
                    'category': response.category if response else None,
                    'health_service': response.health_care_service if response else None,
                    'primary_care': response.primary_health_care if response else None,
                    'secondary_care': response.secondary_health_care if response else None,
                    'tertiary_care': response.tertiary_health_care if response else None
        ...
        return {
            ...
            'uncategorized_count': sum(1 for p in project_details if not p['is_categorized']),
